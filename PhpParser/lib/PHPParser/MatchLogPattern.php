...
class PHPParser_MatchLogPattern
{
        ...
        foreach($root as $key => $value) {
            ...
            if($value instanceof PHPParser_Node) {
                if($value instanceof PHPParser_Node_Stmt_Return && $value->expr instanceof PHPParser_Node) {
                    $match_arr[]=$value->getType() . '--' . $value->expr->getType();
                } else {
                    $match_arr[]=$value->getType();
                }
                
                $temp_is_log=array(false,$value->getLine());
                if($value->getType()=='Expr_FuncCall') {
                    if(preg_match("/log/i", $value->name->parts[0])) {
                        $temp_is_log=array(true,$value->getLine());
                    }
                }
                elseif($value->getType()=='Expr_StaticCall') {
                    if(preg_match("/log/i", $value->class->parts[0])) {
                       $temp_is_log=array(true,$value->getLine());
                    }
                } 
                $is_log_arr[]=$temp_is_log;
                
                #$match_arr[]=$value->getType();
            }
            elseif(is_array($value)) {
                $match_arr[]="array";
                $is_log_arr[]=array(false,$value->getLine());
            }
            else{
                $match_arr[]=" ";
                $is_log_arr[]=array(false,$value->getLine());
          ...
          for($j=0; $j<$len_match_arr - $len_log_arr + 1; $j++) {
            ...
            if(count($check_result) == 0) {
               ...
               //if(abs($log_line - $sub_match_line[$log_index]) > 2) {
                  ...
                  if(isset($is_log_arr[$j+$log_index+1]) && $is_log_arr[$j+$log_index+1][0]) {
                    #print $temp_match_line . " ";
                    $temp_match_line = $is_log_arr[$j+$log_index+1][1];
                    #continue;
                  }
                  if(isset($is_log_arr[$j+$log_index]) && $is_log_arr[$j+$log_index][0]) {
                    #print $temp_match_line . " ";
                    $temp_match_line = $is_log_arr[$j+$log_index][1];
                    #continue;
                  }

                  // if($temp_match_line == 368) {
                  //   print_r($match_arr);
                  //   print_r($match_line);
                  //   print_r($match_up_line);
    ...
    private function find_log_pattern($pnode,$ptype_arr) {
        ...
        foreach($this->g_log_arr as $index => $log_arr) {
            if($grand_parent_arr[0] == $log_arr["grand_parent"] && 
                abs($grand_parent_arr[1] - $log_arr["grand_parent_len"]) < 6) {
    	...
    	if(count($grand_parent_match_list) > 0) {
            ...
            for ($i=0; $i < count($grand_parent_match_list); $i++) { 
                ...
                if( $parent_arr[0] == $this->g_log_arr[$j]["parent"] &&
                    abs($parent_arr[1] - $this->g_log_arr[$j]["parent_len"]) < 4 ) {
                    $parent_match_list[]=$j;
                }
            }
    ...
    public function get_sug_line2log_index_arr() {
